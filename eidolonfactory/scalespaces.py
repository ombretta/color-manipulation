...
#        return np.fft.irfft2((dataPlaneFFT * kernelFFT)) 
    ...
    def Kernel(self, xOrder, yOrder, w, h, sigma):
        ...
        column = ((-1.0/(sigma * np.sqrt(2)))**yOrder) * self.HermitePolynomial(yOrder, v/(sigma * np.sqrt(2))) * self.Gaussian(v, sigma)
        u = np.array(list(range(w//2 + 1)) + list(range(w//2-w + 1, 0)))
        row = ((-1.0/(sigma * np.sqrt(2)))**xOrder) * self.HermitePolynomial(xOrder, u/(sigma * np.sqrt(2))) * self.Gaussian(u, sigma)
...
class DOGScaleSpace(object):
    ...
    The nature of this decomposition is that it is by "edges" in the phenomenological
...
#    SZ = 16
    SZ = 512
    MIN_SIGMA = 1/ np.sqrt(2) #// 1f/sqrt(2f)
    MAX_SIGMA = SZ/4.0
    SIGMA_FACTOR = np.sqrt(2)  #// sqrt(2f)
